...
import { setTransferChain, setTransferToken } from '@/store/features/ChainSlice';


const TransferTokenSelectDialog: React.FC = () => {
const dispatch=useDispatch();
    ...
    useEffect(()=>{
        ...
        const restTokens=Tokens.filter((token:TokensType)=>token.chainname!==originChain?.chainname)
    ...
    },[searchText,originChain])

    const handleOpen = () => setOpen((cur) => !cur);

    const listitemClick=(token:TokensType)=>{
       dispatch(setTransferToken(token));
       dispatch(setTransferChain(Networks.find((network:NetworkType)=>network.chainname==token.chainname)));
    ...
    return (
        <div>
            <div className="flex flex-1 bg-[#421851] w-[256px] gap-2 items-center hover:cursor-pointer hover:bg-[#50075C]" onClick={handleOpen}>
                ...
                {
                    transferToken?.chainname ? (
                        <Image className="mx-4  rounded-full" src={transferToken.image} width={40} height={40} alt="chainname" />
                ...
                <div className="flex flex-col">
                    ...
                    <div className="flex flex-1 gap-1 items-center">
                        <Typography variant="paragraph" color="white">{transferToken?.chainname ? `${transferToken.unit}` : `Select`}</Typography>
...
export default TransferTokenSelectDialog;